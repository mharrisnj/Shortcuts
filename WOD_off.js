...
try {
	...
	async function getWord() {
		...
		req.body = JSON.stringify({
			...
			messages: [{ role: "user", content: "Give me today's word. It must begin with " + randomLetter + ". Format your response exactly as follows with no variation: first the full dictionary entry, then a line containing only ---, then a line containing only the word plus ^ plus the definition. The definition starts with a capital letter and no leading space" }]
		});
